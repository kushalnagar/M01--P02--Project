...
class Node:
    ...
    def get_data(self, key):
        ...
        node = self._vnode_map.get_assigned_node(key)
        value = self._node_dict[node]._data_store[key]
        return value
    ...
    def set_data(self, key, value, force=False):
        if (force):
            ...
            node = self._vnode_map.get_assigned_node(key)
            self._node_dict[node]._data_store[key] = copy.deepcopy(value)
    ...
    def add_new_node(self, new_node_name, new_node):
        ...
        v_node_map = self._vnode_map.vnode_map
        for key in v_node_map:
          if v_node_map[key] == self._name:
        ...
        for vnode_id in local_vnode_slice:
          if vnode_id not in transfer_dict:
            transfer_dict[vnode_id]={'target_node':new_node_name, 'keys':[]}

        for node_name in self._node_dict:
          node = self._node_dict[node_name]
          for key in node.get_keys():
            v_node_for_key = key%self._TOTAL_VIRTUAL_NODES
            if v_node_for_key in local_vnode_slice:
              transfer_dict[v_node_for_key]['keys'].append(key)

        # Transfer the remapped keys to the new node
        print("Before transfer -- Adding new node")
        print(transfer_dict)
        self.transfer_keys(transfer_dict)
        print("After transfer -- Adding new node")
        print(transfer_dict)
    ...
    def remove_current_node(self, new_node_dict):
        ...
        #                }
        # Here 23 and 96 are examples of vnode ids
        for vnode_id in transfer_node_mapping:
          if vnode_id not in transfer_dict:
            transfer_dict[vnode_id]= {'target_node': transfer_node_mapping[vnode_id], 'keys': []}


        for key in self.get_keys():
          v_node_for_key = key%self._TOTAL_VIRTUAL_NODES
          if v_node_for_key in transfer_node_mapping:
            transfer_dict[v_node_for_key]['keys'].append(key)
